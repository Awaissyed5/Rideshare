...
import 'package:rideshare_driver/Views/tabPages/chat_screen.dart';
...
//  late final TextEditingController _tripNameController;
  ...
  double _costPerMeter = 0.1;
  double _costPerMinute = 0.2;
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('Total Fare: ${_totalCost.toInt()}',
  ...
  void _startTrackingSimulation() {
    ...
    accelerometerEvents.listen((AccelerometerEvent event) {
      double acceleration =
          event.y; // Choose the appropriate axis for speed calculation

      // Convert acceleration to speed
      // Adjust the conversion factor as needed based on your requirements
      // You may need to experiment with different values to get accurate results
      speed = acceleration * conversionFactor;
    });
    ...
    _simulationTimer =
      ...
      if (_lastPosition != null && _destination != null) {
        ...
        // Only update if the distance is greater than 0
  ...
  void _updateDate() async {
    ...
    if (pickedDate != null) {
      ...
      // Update the date in the database using the pickedDate value
      FirebaseDatabase.instance.ref("trips").child(widget.trip.tripID).update({
  ...
  void _updateTime() async {
    ...
    if (pickedTime != null) {
      ...
      // Update the time in the database using the pickedTime value
      FirebaseDatabase.instance.ref("trips").child(widget.trip.tripID).update({
